...
if __name__ == "__main__":
    ...
    model_dir_path = "logs/MDMTN_MM_logs/MGDA_model_logs/model_states" + "/" + dt_string
    os.mkdir(model_dir_path)

    for i in range(model_repetitions):
        print(f"######## Repetition {i+1}/{model_repetitions} ########")
        model_multi = MDMTNmgda_MultiTaskNetwork_I(batch_size, device=device, static_a = [False, None])
        ...
        for epoch in tqdm(range(training_epochs)):
            ...
            train_loss = train_multi(torch.tensor(X_train, dtype=torch.float32, device=device),
                                                          ...
                                                          loss_fn, batch_size, device=device, archi = archi, img_shp = (28, 28, 1))
