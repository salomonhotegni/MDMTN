...
class MDMTNmgda_MultiTaskNetwork_II(nn.Module):
    ...
    def __init__(self, batch_size, device="cpu", static_a = [True, None]):
        super().__init__()
        self.num_of_tasks = 2
        self.shared = MultiTNShared(batch_size).to(device)
        self.taskOutput = dict([(f"task_{t}", MultiTNTaskSpecific().to(device)) for t in range(self.num_of_tasks)])
        self.monitors = dict([(f"monitor_{t}", TaskMonitor(batch_size).to(device)) for t in range(self.num_of_tasks)])
...
class MultiTNShared(nn.Module):
    ...
    def __init__(self, batch_size):
        ...
        self.batchnorm_sh1 = nn.BatchNorm2d(16)
        self.conv2 = nn.Conv2d(16, 32, 3)
        self.batchnorm_sh2 = nn.BatchNorm2d(32)
        self.fc = nn.Linear(1152, 32)
    ...
    def forward(self, x):
        x = x.transpose(4, 2)
        x = x.reshape([self.batch_size, 3, 32, 32])
        x = self.conv1(x)
        x = self.relu(self.batchnorm_sh1(self.max_pool(x)))
        x = self.conv2(x)
        x = self.relu(self.batchnorm_sh2(self.max_pool(x)))
        x = x.reshape([self.batch_size, 1152])
...
class TaskMonitor(nn.Module):
    ...
    def __init__(self, batch_size):
        ...
        self.batchnorm_mon1 = nn.BatchNorm2d(32)
        self.fc = nn.Linear(7200, 32)
    ...
    def forward(self, x):
        ...
        x = x.reshape([self.batch_size, 3, 32, 32])
        x = self.conv1(x)
        x = self.relu(self.batchnorm_mon1(self.max_pool(x)))
        x = x.reshape([self.batch_size, 7200])
        return self.relu(self.fc(x))
